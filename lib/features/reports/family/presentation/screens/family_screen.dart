...
class _FamilyScreenState extends ConsumerState<FamilyScreen> {
  ...
  void _onSearchChanged(String value) {
    _loadMoreTimer?.cancel(); 
    _loadMoreTimer = Timer(const Duration(seconds: 2), () {
      ref.read(familyProvider.notifier).loadMore(search: value);
      ref.read(familyProvider.notifier).loadInitial(search: value);
    });
  }
            ...
            children: [
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemCount:
                      state.family.length + (state.isLoading ? 1 : 0),
                  itemBuilder: (context, index) {
                    if (index == state.family.length) {
                    ...
                    final family = state.family[index];
                    return FamilyCard(
                      family: family,
                    );
                  },
